...
void verifica(int num, int escolha) //Função verificadora de número retangular.
{
    int soma = 0, qtd = 0, contador;

    for (contador = 0; soma < num; contador = contador + 2) //Soma até o valor do número informado.
    {
        soma = (soma + contador);
        qtd++; //Quantidade de números pares até o número informado.
    }

    if (escolha == 1) //Verifica se o número é retangular.
    {
        if (soma == num && soma != 0) //Validação de número retangular.
        {
            ...
            if (soma != 2) //Validação para número maior que 2.
            {
                ...
                while (qtd > 2);//Decrescimo da quantidade de números até 2.

                contador = (contador + 2); //Contador assume último valor da soma.
                printf("%d = %d\n", contador, num); //Num é o valor retangular.
            }
            else
            {
                printf("%d = %d", soma, num);//Validação para número informado igual a 2.
            }
        }
        else
        {
    ...
    {
        if (soma == num && soma != 0) //Validação para número diferente de 0 e validação de número retangular.
        {
            ...
            {
                printf("%d = %d\n", soma, num); //Número retangular igual a 2.
            }
        }
    }
}
...
int main(void) //Função principal
{
    ...
    int opcao, numero, inf, sup;//inf e sup referem-se aos limites; opcao é usada no switch case e o numero é o valor informado.
    ...
    do
    {
        ...
        case 2: //Mostra os numeros retangulares que pertencem a um intervalo
            ...
            while (inf > sup); //Validação de intervalo.

            for (inf; inf <= sup; inf++)//Execução da função de verificação por todo o intervalo.
            {
                verifica(inf, opcao);
            }
        ...
        printf("\nDeseja repetir a execucao do programa? <S/s ou N/n>\n");//Repetição do programa
